...
const generateRandomCourses = (count, specializationCount) => {
    if (!Number.isInteger(specializationCount) || specializationCount <= 0) {
        console.warn(`⚠️ Invalid specializationCount: ${specializationCount}. No courses will be generated.`);
        return [];
    }
    return Array.from({ length: count }, (_, i) => ({
        recordType: 'course',
...
const generateRandomUserCourses = (count, userCount, courseCount) => {
    ...
    const generateCsvData = async () => {
        try {
            ...
            const specializationCount = 50;
            if (specializationCount <= 0) {
                console.error('❌ specializationCount must be greater than 0.');
                process.exit(1);
            }
            ...
            const weekCount = 50;
            const taskCount = 50;
            const deadlineCount = 50;
            const userCourseCount = 50;
            ...
            console.log(`Generated specializationCount: ${specializationCount}`);
